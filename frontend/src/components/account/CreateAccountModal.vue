    ...
    >
      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity'" class="space-y-4">
        <div>
          ...
          <div>
            <label class="input-label">{{ apiCredentialRequiredLabel }}</label>
            <input
              v-model="apiKeyValue"
              type="password"
              required
          ...
          <div
            ...
            class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700 dark:border-slate-800/40 dark:bg-slate-900/20 dark:text-slate-300"
          >
            Copilot 使用 GitHub Token 手动接入，不走前端 OAuth 授权流程。
          </div>
...
const baseUrlHint = computed(() => {
  ...
  if (form.platform === 'copilot') return 'GitHub Copilot upstream base URL. Default: https://api.githubcopilot.com'
...
const apiKeyHint = computed(() => {
  ...
  if (form.platform === 'copilot') return 'Enter a GitHub access token for Copilot token exchange.'
  return t('admin.accounts.apiKeyHint')
})

const apiCredentialRequiredLabel = computed(() => {
  if (form.platform === 'copilot') return 'GitHub Token'
